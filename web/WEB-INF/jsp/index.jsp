...
<body>
	...
	<!-- 轮播图 -->
		<div id="myCarousel" class="carousel slide" style="height: 590px">
			<!-- 轮播（Carousel）指标 -->
...
<script type="text/javascript">
	...
	$(function() {
		$.ajax({
			type : "post",
			dataType:"json",
			url : "${pageContext.request.contextPath}/requestAds",
			success : function(data) {//返回json结果
				/*console.log(data);*/
				var html = '';
				$.each(data, function(i, item) {
					if(i==0)
						html+='<div class="item active" ><img src="/pic/ads/banner2.jpg" style="height:590px;width:1520px"></div>'
					else
						html+='<div class="item"><img src="/pic/'+item.pic+'" style="height:590px;width:1520px"></div>';
				});
	...
	$(function() {
		$.ajax({
			type : 'post',
			url : '${pageContext.request.contextPath}/requestScenic',
	...
	$(function() {
		$.ajax({
			type : 'post',
			url : '${pageContext.request.contextPath}/requestSpecialty',
			//请求是key/value这里不需要指定contentType，因为默认就 是key/value类型
			//contentType:'application/json;charset=utf-8',
			//数据格式是json串，商品信息
			...
			success : function(data) {//返回json结果
				...
				$.each(data, function(i,item) {
					if(i == 0){
						html+='<li><a  href="${pageContext.request.contextPath}/specialty/getSpecialty?tnum='+item.areanum+'&id='+item.contentid+'"> <img title="" class="scale-effect" style="height:325px;width:790px;" src=/pic/'+item.pic+' /><p>'+item.title+'</p></a></li>'
					}else{
						html+='<li><a  href="${pageContext.request.contextPath}/specialty/getSpecialty?tnum='+item.areanum+'&id='+item.contentid+'"> <img title="" class="scale-effect" style="height:325px;width:390px;" src=/pic/'+item.pic+' /><p>'+item.title+'</p></a></li>'
					}
				});
	...
	$(function() {
		$.ajax({
			type : 'post',
			url : '${pageContext.request.contextPath}/requestTravels',
			//请求是key/value这里不需要指定contentType，因为默认就 是key/value类型
			//contentType:'application/json;charset=utf-8',
			//数据格式是json串，商品信息
	...
	$(function() {
		$.ajax({
			type : 'post',
			url : '${pageContext.request.contextPath}/requestInformation',
			//请求是key/value这里不需要指定contentType，因为默认就 是key/value类型
			//contentType:'application/json;charset=utf-8',
			//数据格式是json串，商品信息
			data : 'value=11',
